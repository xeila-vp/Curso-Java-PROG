...
public class DinamicArray<T> implements Iterable<T> {
    ...
    private UnaryOperator<T> cloner=null;
    private int size=0;
    
        
    public static <T> DinamicArray<T> copyOf(T[] data, UnaryOperator<T> copier) {
    ...
    * Modificar 'data' despois desta chamada implica a modificación do DinamicArray que estamos a crear.
     ...
     * @param copier
    */
    public DinamicArray(T[] data,UnaryOperator<T> copier) {
        this.data=data;
    ...
    public DinamicArray(UnaryOperator<T> copier) {
    ...
    public DinamicArray(T[] data) {
        this(data,UnaryOperator.identity());
    }
    ...
    public T set(int index, T element) {
        ...
        T old=(T)data[index];
        data[index]=cloner.apply(element);
        return old;
    }
    
    public T get(int idx) {
        if (idx < 0 || idx >= size) throw new IndexOutOfBoundsException("Índice " + idx + " fóra de rango (size=" + size + ")");
        return cloner.apply((T)data[idx]); // Necesito un cast porque data e de tipo Object[]. E seguro porque so permito gardar elementos de tipo T
    }
    ...
    public boolean add(T element) {
        ...
        data[size] = cloner.apply(element);
        size++;
        return true;
    }
    ...
    public T[] process(Function<T,T> processor,Class<? extends T> clazz) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            newdata[i] = processor.apply(cloner.apply(elemento));
        }
        return newdata;
    }
